...
void ContactDialog::setCollectionOptions(const QList<QPair<QString, QString>> &collections, int currentIndex)
{
    ui->comboBoxCollection->clear();
    for (const QPair<QString, QString> &collection : collections)
    {
        ui->comboBoxCollection->addItem(collection.first, collection.second);
    }
    if (currentIndex >= 0 && currentIndex < ui->comboBoxCollection->count())
    {
        ui->comboBoxCollection->setCurrentIndex(currentIndex);
    }
    ui->comboBoxCollection->setEnabled(ui->comboBoxCollection->count() > 1);
    ui->labelCollection->setVisible(!collections.isEmpty());
    ui->labelCollection->setBuddy(ui->comboBoxCollection);
    ui->comboBoxCollection->setVisible(!collections.isEmpty());
    ui->lineEditCollection->hide();
}
...
{
    const ContactFields value = data();
